...
function BudgetCard({name, amount, max, gray, onAddExpenseClick, hideButtons, onViewExpensesClick}) {

    const classnames = [];
    if(amount > max) {
        classnames.push("bg-danger", "bg-opacity")
    }else if (gray) {
        ...
        <Card className={classnames.join(" ")}>
            <Card.Body>
                ...
                {!hideButtons && <Stack direction="horizontal" gap="2" className="mt-4">
                    ...
                    <Button 
                        variant="outline-secondary" onClick={onViewExpensesClick}>
                        View Expenses
                    </Button>
                </Stack>}
            </Card.Body>
